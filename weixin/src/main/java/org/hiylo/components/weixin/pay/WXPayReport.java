...
public class WXPayReport {
    ...
    private ThreadFactory wxPayReportThreadFactory = new ThreadFactoryBuilder()
            .setNameFormat("wxPayReport").build();
    ExecutorService pool = new ThreadPoolExecutor(1, 10,
            0L, TimeUnit.MILLISECONDS,
            new LinkedBlockingQueue<>(128), wxPayReportThreadFactory, new ThreadPoolExecutor.AbortPolicy());

    private WXPayReport(final AbstractWXPayConfig config) {
        this.config = config;
     ...
     * @return
     */
    public static WXPayReport getInstance(AbstractWXPayConfig config) {
        if (INSTANCE == null) {
